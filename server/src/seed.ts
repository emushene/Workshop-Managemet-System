...
const seed = () => {
  ...
  bcrypt.hash('admin', saltRounds, (err: Error | null, adminPassword: string) => {
    ...
    db.serialize(() => {
      db.run('DROP TABLE IF EXISTS ServiceItemParts');
      ...
      db.run('PRAGMA foreign_keys = ON;'); // Re-enable foreign key checks
      ...
      const customerStmt = db.prepare("INSERT OR IGNORE INTO Customers (name, address, telephone, email) VALUES (?, ?, ?, ?)");
      ...
      const jobStmt = db.prepare("INSERT OR IGNORE INTO Jobs (customerId, itemDescription, serviceDescription, jobType, status, vehicleMake, vehicleModel, vehicleYear, servicePrice, dateBookedIn, dateExpectedOut) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
      ...
      const inventoryStmt = db.prepare("INSERT OR IGNORE INTO Inventory (name, quantity, price) VALUES (?, ?, ?)");
      ...
      const invoiceStmt = db.prepare("INSERT OR IGNORE INTO Invoices (jobId, totalAmount, amountPaid, discountAmount, dateCreated, status) VALUES (?, ?, ?, ?, ?, ?)");
